    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal/40 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-md bg-white border border-sage-300 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <form onSubmit={handleCreateGroup} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-4 sm:p-5 space-y-3.5 border-b border-sage-200 bg-white">
            {/* Group Name */}
            <div>
              ...
              <input
                ...
                className="w-full bg-sage-50 text-charcoal placeholder-charcoal-50 border border-sage-300 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-forest focus:ring-2 focus:ring-forest/20 transition-all"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-sage-50 text-charcoal placeholder-charcoal-50 border border-sage-300 rounded-xl pl-10 pr-4 py-2 text-xs focus:outline-none focus:border-forest focus:ring-2 focus:ring-forest/20 transition-all"
                />
              </div>
            </div>
            ...
            {selectedUsers.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {selectedUsers.map((u) => (
                  <span
                    key={u._id}
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-forest/10 text-forest text-xs font-semibold border border-forest/20"
                  >
                    <span>{u.name}</span>
                    <button
                      type="button"
          ...
          {/* User Results List */}
          <div className="overflow-y-auto flex-1 p-3 space-y-1 divide-y divide-sage-200/60 bg-white">
            {users.length === 0 ? (
              <div className="py-8 text-center text-charcoal-50 text-xs">
              ...
              users.map((u) => {
                ...
                return (
                  <div
                    key={u._id}
                    onClick={() => toggleSelectUser(u)}
                    className="flex items-center justify-between p-2.5 rounded-2xl hover:bg-sage-100 cursor-pointer transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <img src={u.avatar} alt={u.name} className="w-8 h-8 rounded-xl object-cover border border-sage-300" />
                      <div>
          ...
          <div className="p-4 border-t border-sage-200 bg-sage-50">
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-3 rounded-2xl bg-forest hover:bg-forest-600 active:bg-forest-700 text-white font-bold text-sm shadow-sm transition-all duration-200 active:scale-[0.98] disabled:opacity-50"
            >
